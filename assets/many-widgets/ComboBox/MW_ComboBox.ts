...
export class MWComboBox extends Component {
    ...
    private _spawnItems() {
        let height = this._item.getComponent(UITransformComponent)!.height;
        this._content.getComponent(UITransformComponent)!.height = height * this._itemsTextArray.length;
        ...
        for (let i = 0; i < this._itemsTextArray.length; i++) {
            ...
            //todo add计算子节点位置
            item.translate(new Vec3(0, -height * i, 0));
